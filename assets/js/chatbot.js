...
form.addEventListener('submit', async e => {
  ...
  messages.innerHTML += `<li>You: ${text}</li>`;
  input.value = '';
  ...
  const resp = await fetch('/chat/api/', {
    ...
    body: JSON.stringify({
      ...
      user_info:  { height_cm:170, weight_kg:60, body_type:'athletic' },
      past_orders: ['sku123','sku456'],
      user_message: text         // ← this key must match the view
    })
  });
  ...
  const { session_id, reply } = await resp.json();
  localStorage.setItem('chat_session', session_id);

  messages.innerHTML += `<li>Elle: ${reply}</li>`;
  messages.scrollTop = messages.scrollHeight;
});
